...
def clas_g(result):
    if result.summary()['params'][0][1] <-1e-4:
...
def clas(px,channel,plot=False):
    ...
    # Secciones para wid
    secc1 = Molines_A_df[px].iloc[channel[0]+150:channel[1]-150]
    ...
    s = np.sum(cube[channel[0]:channel[1],:,:],axis=0)[int(px.split('_')[1]),int(px.split('_')[2])]
...
'''
cube,Molines_A_df = Cube_to_df('IRAS_15445-5449',25)
...
plt.contour(sumaCO,levels=(1.6,2,3,4,5),colors='gray')
