...
for ax1, line,line_name in zip(axes, lines,line_names):
    ...
    ax1.contour(x_offsets, y_offsets, x_z_projection, levels=np.array([2,4.2,5.5])*np.std(x_z_projection), colors='k', linewidths=0.7, alpha=0.7)
        
    
    ax1.invert_yaxis()

    ax1.set_xlabel('J2000 DEC offset (′′)', fontsize=17)
...
plt.savefig('/Users/mac/Tesis/Cut_Cubes/Emission_Figures/PV diagrams/PV diagram DEC (offset)', dpi=300, bbox_inches='tight')  # 'dpi=300' aumenta la resolución
