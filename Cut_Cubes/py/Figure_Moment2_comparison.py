...
contornosm2 = [[0.23, 0.28, 0.4, 0.65, 0.84, 0.99],[0.2, 0.25, 0.35, 0.5,0.65, 0.8, 0.99],[0.29, 0.38, 0.5,0.66, 0.85]]
...
m2_13co = moment2_13CO(path_13co)
vmin = 5
vmax = 66
...
for k in range(len(names)):
    ...
    im2 = ax2.imshow(m2_escalar, origin='lower',cmap='terrain_r',
    ...
    # --- ticks grandes ---
    ax2.tick_params(axis='both', which='major', labelsize=17, length=6, width=1.5, direction='in')
    ax2.tick_params(axis='both', which='minor', labelsize=16, length=4, width=1, direction='in')

    # --- título ---
    ax2.set_title(line_names[k], fontsize=25, pad=10)

    # Contornos en el segundo gráfico
    contours2 = ax2.contour(x_offsets, y_offsets, m2_escalar, levels=np.array(contornosm2[k]) * np.nanmax(m2.value), 
                            linewidths=0.7, colors='black')
    ax2.clabel(contours2, inline=True, fontsize=12)

    # Contornos sobre el segundo gráfico
    cont2 = ax2.contour(x_offsets, y_offsets, m0.value, levels=np.array(contornos[k]) * round(np.nanmax(m0.value), 1), 
                        linewidths=2, colors='red', linestyles='--')

    # Leyenda
    dust_contour_legend = Line2D([], [], color='red', linestyle='--', linewidth=2,
                                 label='Moment 0 Contours')
    ax2.legend(handles=[dust_contour_legend], fontsize=17)
    ax2.minorticks_on()
    ax2.tick_params(axis='both', which='major', labelsize=17)  # números más grandes
    ax2.tick_params(axis='both', which='minor', labelsize=16)  # si tienes minor ticks
    ...
    rec = FancyBboxPatch(
        (-0.32,-0.45),  # esquina inferior izquierda
        width,
        height,
...
cbar.ax.tick_params(labelsize=17, length=6, width=1.5)
cbar.set_label('(km/s)', fontsize=18)
plt.tight_layout()
    
fig.savefig('Moment2_comparison.png', dpi=300, bbox_inches='tight')  # 'dpi=300' aumenta la resolución
