...
contours1 = ax1.contour(x_offsets, y_offsets, m0.value,
                        levels=np.array([0.45, 0.6, 0.8, 0.9]) * round(np.nanmax(m0.value), 1),
...
plt.savefig('/Users/mac/Tesis/Cut_Cubes/Emission_Figures/SO2_21_21_moment0_44kpc.png', dpi=300, bbox_inches='tight')  # 'dpi=300' aumenta la resolución
...
im2 = ax2.imshow(m2_escalar, origin='lower', vmin=7, vmax=46, cmap='terrain_r',extent=extent)
...
contours2 = ax2.contour(x_offsets, y_offsets, m2_escalar, levels=np.array([0.29, 0.38, 0.5,0.66, 0.85]) * np.nanmax(m2.value), 
                        linewidths=0.7, colors='black')
ax2.clabel(contours2, inline=True, fontsize=12)

# Contornos sobre el segundo gráfico
cont2 = ax2.contour(x_offsets, y_offsets, m0.value, levels=np.array([0.45, 0.6, 0.8, 0.9]) * round(np.nanmax(m0.value), 1), 
                    linewidths=2, colors='red', linestyles='--')
...
rec = FancyBboxPatch(
    (-0.2,-0.32),  # esquina inferior izquierda
    width,
    height,
...
plt.savefig('/Users/mac/Tesis/Cut_Cubes/Emission_Figures/SO2_21_21_moment2_44kpc.png', dpi=300, bbox_inches='tight')  # 'dpi=300' aumenta la resolución
...
ax3.set_yticklabels([-1,0.0,0.5,1.0,1.5,2.0,2.5,3.0])
...
ax3.legend(fontsize=17,loc='upper right')
# Ajustar el layout para evitar que los gráficos se superpongan
plt.tight_layout()

output_filename = '/Users/mac/Tesis/Cut_Cubes/Emission_Figures/SO2_21_21_mean_emission.png'  # Cambia la extensión si prefieres otro formato como .pdf, .svg, etc.
